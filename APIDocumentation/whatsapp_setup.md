# Configuración Inicial de Evolution API

Dado que Evolution API se ejecuta localmente (en tu contenedor `iye-evolution-api`), necesitas crear la instancia y escanear el código QR por primera vez para vincular tu número de WhatsApp. 

Sigue estos pasos en orden. Asegúrate de que los contenedores de Docker estén corriendo (`docker compose up -d`).

## Paso 1: Crear la Instancia

Por defecto en el código de backend utilizamos el nombre de instancia `mi_whatsapp`. Debemos crearla en Evolution API.
...
Si ya habías creado la instancia pero perdiste la sesión, puedes solicitar un nuevo código QR con este comando:
...
Si la respuesta tiene un `"state": "open"`, significa que **ya estás listo** para empezar a mandar mensajes desde el Backend usando el endpoint `/api/whatsapp/send`.
